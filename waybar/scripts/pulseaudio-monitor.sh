...
output_all() {
  ...
  if ! echo "$SINK_NAME" | grep -qiE "headphone|earpiece"; then
    if [ "$MUTE" = "yes" ]; then
      SPEAKERS_OUTPUT="{\"text\":\"<span size='14pt' weight='600' rise='-3pt'></span>\"}"
    else
      ...
      SPEAKERS_OUTPUT="{\"text\":\"<span size='13pt' rise='-1pt' weight='400'>$ICON</span> <span size='12pt'>${VOLUME}%</span>\"}"
    fi
  else
